 ...
 * - Disable: PI_SPEAK_OFF=1 (env, fixed at launch)
 * - Mute toggle: echo 1 > ~/.pi/speak.mute (checked before EVERY reply,
 *   takes effect immediately, no reload needed; rm the file to unmute)
 *
 * 朗读控制 (需求3: 打断/跳过):
...
export default function (pi: ExtensionAPI) {
	...
	const MUTE_FILE =
		process.env.PI_SPEAK_MUTE_FILE ??
		(process.env.HOME ?? "/tmp") + "/.pi/speak.mute";
	...
	/** 静音开关: 文件 ~/.pi/speak.mute 内容为 "1" 即静音 (每次回复前检查, 无需 reload) */
	const isMuted = (): boolean => {
		try {
			return readFileSync(MUTE_FILE, "utf8").trim() === "1";
		} catch {
			return false;
		}
	};
	...
	const playChunk = (tmp: string) =>
		new Promise<void>((resolve) => {
			const p = execFile("/usr/bin/afplay", [tmp], { timeout: 600000 }, () =>
				resolve(),
			);
			currentChild = p;
	...
	const runQueue = async () => {
		...
		try {
			while (queue.length > 0 && !cancelled) {
				...
				const usable =
					made !== null && existsSync(tmp) && statSync(tmp).size >= 1000;
				if (!usable) {
					try {
						unlinkSync(tmp);
					} catch {
	...
	pi.on("agent_end", async (event, _ctx) => {
		log(
			"agent_end 事件: willRetry=" +
				String(event.willRetry) +
				", 消息数=" +
				String(event.messages?.length ?? 0),
		);
